...
from tunachain_processor.carchain_payload import CarchainPayload
from tunachain_processor.carchain_state import CarchainState
from tunachain_processor.carchain_state import CARCHAIN_NAMESPACE


LOGGER = logging.getLogger(__name__)


class CarchainTransactionHandler(TransactionHandler):
    ...
    @property
    def namespaces(self):
        return [CARCHAIN_NAMESPACE]

    def apply(self, transaction, context):
        header = transaction.header
        signer = header.signer_public_key

        payload = CarchainPayload(transaction.payload)
        state = CarchainState(context)
